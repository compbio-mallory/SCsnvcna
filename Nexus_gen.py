...
def to_Nexus(bin_array, samples, output_address, logfile):
...
def main():
	...
	infile = sys.argv[1]
	outfile = sys.argv[2]
	logfile = outfile.split(".")[0]
	(bin_array,samples) = Parse_Ginkgo(address=infile)
	to_Nexus(bin_array=bin_array,samples=samples,output_address=outfile, logfile = logfile)
